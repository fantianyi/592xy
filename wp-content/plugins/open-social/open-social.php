...
function open_social_login_html($atts=array()) {
	...
	$html .= '<div class="login_button login_icon_blank"><a href="'.wp_login_url($_SERVER['REQUEST_URI']).'">'.__('Log In').'</a></div>';
	$html .= '<div class="login_button login_icon_blank"><a href="'.wp_registration_url($_SERVER['REQUEST_URI']).'">'.__('Register').'</a></div>';
	$html .= '</div>';
	return $html;
}
...
function open_social_profile_html(){
	...
	$html = "";
	//$html = '<a href="'.(current_user_can('manage_options')?admin_url():(get_edit_user_link($current_user->ID)).'?from='.esc_url($_SERVER["REQUEST_URI"])).'">'.get_avatar($current_user->ID).'</a>';
	//$html .= '<a href="'.$current_user->user_url.'" target="_blank" title="'.__( 'Website' ).'">'.$current_user->display_name.'</a>&nbsp&nbsp&nbsp&nbsp';
	$html .= '<a href="'.get_edit_user_link($current_user->ID).'?from='.esc_url($_SERVER["REQUEST_URI"]).'" title="'.__('Edit My Profile').'">'.get_avatar($current_user->ID).$current_user->display_name.'</a><br />';
