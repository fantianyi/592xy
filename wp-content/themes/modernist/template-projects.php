...
public static function encode($utf8Data, $sRetFormat='head'){
    ...
    for ($i=0, $iLoop=strlen($sGBK); $i<$iLoop; $i++) {
        $iChr = ord($sGBK{$i}); 
        if ($iChr>160 && isset($sGBK{$i+1}) )
            $iChr = ($iChr<<8) + ord($sGBK{$i+1}) - 65536;
        if ('head' === $sRetFormat || 'first' === $sRetFormat)
    ...
    if ('head' === $sRetFormat)
        return implode('', $aBuf);
    else if ('first' === $sRetFormat)
        return implode('', $aBuf)[0];
    else
        return implode(' ', $aBuf);
}
		...
		<div class="content">
			<div class="inner_content">
				<div class="projects_field widget widget_anim">
					...
					$initials = array("a", "b", "c", "d", "e", "f", "g", "h", "j", "k", "l", "m", "n", "p", "q", "r", "s", "t", "v", "w", "x", "y", "z"); ?>
                    <div class="initial">
                        <?php foreach($terms as $term): ?>
							<a class="filter" href="<?php echo $term->term_id; ?>"><?php echo $term->name; ?></a>
						<?php endforeach; ?>
                        <?php foreach($initials as $initial): ?>
                            <a class="filter" href="<?php echo $initial; ?>"><span><?php echo strtoupper($initial); ?></span></a>
					    <?php endforeach; ?>
					</div>
                    <?php
					if(count($terms)):
					...
					<div class="project_holder widget_anim">
							<div class="projects_box">
								...
								$category = array();
								$xy_url = get_field("xy_apple_download_url");
								$android_url = get_field("android_download_url");
								...
								$cats = implode(",",$cats);
								$cats = $cats . "," . CUtf8_PY::encode(get_the_title(), "first");
								//echo '逍遥:'.CUtf8_PY::encode('逍遥', "first");
								?>
								<div class="project" style="overflow:hidden;" data-categories='<?php echo $cats; ?>'>
									<div style="float: left;width:100px;height:50px;"><img src="<?php echo $image; ?>" /></div>
                                    <h3><?php the_title(); ?></h3>                                    
									<div class="down btn-apple">
                                        <a class="visible-md-block visible-lg-block" href="javascript:;" onclick="createQrCode('<?php echo $xy_url; ?>');"></a>
										<a class="hidden-md hidden-lg" href="<?php echo $xy_url; ?>"></a>
									</div>
                                    <?php
                                      if ($android_url) { ?>
									<div class="down btn-android">
                                        <a class="visible-md-block visible-lg-block" href="javascript:;" onclick="createQrCode('<?php echo $android_url; ?>');"></a>
										<a class="hidden-md hidden-lg" href="<?php echo $android_url; ?>"></a>
									</div>
                                    <?php } ?>
