...
function optionsframework_load_scripts( $hook ) {
	...
	if ( !wp_script_is( 'wp-color-picker', 'registered' ) ) {
		...
		$colorpicker_l10n = array(
			'clear' => __( 'Clear','Solome' ),
			'defaultString' => __( 'Default', 'Solome' ),
			'pick' => __( 'Select Color', 'Solome' )
		);
    ...
    <div id="optionsframework-metabox" class="metabox-holder">
	    <div id="optionsframework" class="postbox">
			...
			<div id="optionsframework-submit">
				<input type="submit" class="button-primary" name="update" value="<?php esc_attr_e( '保存设置', 'Solome' ); ?>" />
				<input type="submit" class="reset-button button-secondary" name="reset" value="<?php esc_attr_e( '恢复默认', 'Solome' ); ?>" onclick="return confirm( '<?php print esc_js( __( '如果单击“确定”会导致之前所有的设置都丢失，确定要这样做吗？', 'Solome' ) ); ?>' );" />
				<div class="clear"></div>
			</div>
			</form>
		</div> <!-- / #container -->
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['reset'] ) ) {
		add_settings_error( 'options-framework', 'restore_defaults', __( '已恢复默认设置！', 'Solome' ), 'updated fade' );
...
function optionsframework_save_options_notice() {
	add_settings_error( 'options-framework', 'save_options', __( '设置已保存！', 'Solome' ), 'updated fade' );
}
...
if ( ! function_exists( 's_opt' ) ) {

	function s_opt( $name, $default = false ) {
